...
Summary.df <- summarySE(Whole_data, measurevar = "AP_Onset", groupvars = c("Injected_current", "group"), na.rm = T) #Summerize data Mean, SD, SEM 
print(Summary.df)
```
### 3. Prepare figure
```{r, echo=FALSE}
library(ggplot2)
P <- ggplot(Summary.df, aes(x=Injected_current, y=AP_Onset, colour=group, group=group)) + 
    geom_errorbar(aes(ymin=AP_Onset-se, ymax=AP_Onset+se), colour="black", width=.2) +
    geom_line() +
    geom_point(size=5, shape=21, fill="white") + # 21 is filled circle
    labs(x="Injected Current (pA)", y="AP Onset latency (ms)", title="") + #Add axis titles
    scale_colour_hue(name= "",    # Legend label, use darker colors
                     breaks=c("WT", "KI"),
                     labels=c("WT", "KI"),
                     l=40) +                    # Use darker colors, lightness=40
    expand_limits(y=2) +                        # Expand y range
    scale_y_continuous(breaks=0:20*10) +         # Set tick every 5
...
```{r}
LMM.model <- lme(AP_Onset ~ Injected_current*group, 
                 random = ~1|id, 
                 data=Whole_data, 
                 na.action = "na.omit")
```
...
```{r}
a <- aov(AP_Onset~Injected_current+group+group*Injected_current+Error(id), data=Whole_data)
summary(a)
```
```{r}
anova(LMM.model)
