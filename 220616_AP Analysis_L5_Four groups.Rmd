---
title: Comparison of intrinsic excitability of mPFC PN between CHD8 genotypes:::Action
  Potential Analysis
author: "KimYG"
date: "2022 June 17th"
output:
...
```{r echo=FALSE}
AP <- read.delim(file="C:/Users/KimYG/Desktop/3. CHD8/IE/Summary/WT_F_L5/AP analysis.txt",
                   header=T,
                   sep='\t',
                   stringsAsFactors=T)
...
AP$group  <- factor("WT_F_L5")
AP2 <- read.delim(file="C:/Users/KimYG/Desktop/3. CHD8/IE/Summary/WT_M_L5/AP analysis.txt",
                   header=T,
                   sep='\t',
                   stringsAsFactors=T)
AP2$group <- factor("WT_M_L5")
AP3 <- read.delim(file="C:/Users/KimYG/Desktop/3. CHD8/IE/Summary/KO_F_L5/AP analysis.txt",
                   header=T,
                   sep='\t',
                   stringsAsFactors=T)
AP3$group <- factor("KO_F_L5")
AP4 <- read.delim(file="C:/Users/KimYG/Desktop/3. CHD8/IE/Summary/KO_M_L5/AP analysis.txt",
                   header=T,
                   sep='\t',
                   stringsAsFactors=T)
AP4$group <- factor("KO_M_L5")
...
for (n in 2:(length(colnames(Whole_AP))-1)) { 
  ...
  df1 <- filter(select(Whole_AP, colnames(Whole_AP)[n], group), group == "WT_F_L5")
  colnames(df1) <- c("X", "Label")
  
  df2 <- filter(select(Whole_AP, colnames(Whole_AP)[n], group), group == "KO_F_L5")
  colnames(df2) <- c("Y", "Label")
  
  var.result <- var.test(df1$X, df2$Y) #to evaluate the sample variances of the two groups. If p-value is greater than 0.05, then two variances are homogeneous.
